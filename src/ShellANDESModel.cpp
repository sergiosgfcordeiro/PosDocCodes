...
const char* ShellANDESModel::BETA_PROP[10]    = {"beta0","beta1","beta2","beta3","beta4","beta5","beta6","beta7","beta8","beta9"};
const char* ShellANDESModel::THICK_PROP       = "thickness";
const char* ShellANDESModel::ORIENTATION_PROP[3] = {"v1","v2","v3"};    
const char* ShellANDESModel::LARGE_DISP_PROP  = "largeDisp";
      idx_t ShellANDESModel::nodesWritten_    = 0;
Ref<PrintWriter> ShellANDESModel::nodeOut_    = NIL;

//-----------------------------------------------------------------------
//   constructor
//-----------------------------------------------------------------------

ShellANDESModel::ShellANDESModel
     ...
     const Properties&   globdat ) : Super(name)
{
  ...
  myProps.find( beta_[0], BETA_PROP[0] );
  myConf.set  ( BETA_PROP[0], beta_[0] );

  myProps.find( beta_[1], BETA_PROP[1] );
  myConf.set  ( BETA_PROP[1], beta_[1] );

  myProps.find( beta_[2], BETA_PROP[2] );
  myConf.set  ( BETA_PROP[2], beta_[2] );

  myProps.find( beta_[3], BETA_PROP[3] );
  myConf.set  ( BETA_PROP[3], beta_[3] );

  myProps.find( beta_[4], BETA_PROP[4] );
  myConf.set  ( BETA_PROP[4], beta_[4] );

  myProps.find( beta_[5], BETA_PROP[5] );
  myConf.set  ( BETA_PROP[5], beta_[5] );

  myProps.find( beta_[6], BETA_PROP[6] );
  myConf.set  ( BETA_PROP[6], beta_[6] );

  myProps.find( beta_[7], BETA_PROP[7] );
  myConf.set  ( BETA_PROP[7], beta_[7] );

  myProps.find( beta_[8], BETA_PROP[8] );
  myConf.set  ( BETA_PROP[8], beta_[8] );

  myProps.find( beta_[9], BETA_PROP[9] );
  myConf.set  ( BETA_PROP[9], beta_[9] );
...
void ShellANDESModel::configure
...
void ShellANDESModel::getConfig 
...
bool ShellANDESModel::takeAction
...
void ShellANDESModel::getMatrix_
...

{
  ...
  Matrix      Ktheta      ( nodeCount_, nodeCount_ );
  Matrix      Ttheta_u    ( nodeCount_, 3*nodeCount_ );
  ...
  for ( idx_t ie = 0; ie < numElem_; ie++ )
  {
    ...
    // Get the Beta parameters for the ANDES membrane: Beta.

    Vector beta ( 10 );
    for ( idx_t i = 0; i < beta.size(); i++ )
    {
      beta[i] = beta_[i];
    }
    ...
    // Get the Transformation matrix Ttheta_u.

    getTthetau_(Ttheta_u, xcoords, Area);
    ...
    { 
      ...
      // Get the optimum beta_0 membrane ANDES parameter.

      getGamma_(Gamma, stiff);
      beta[0] = (2.0/Gamma)-(3.0/2.0);
      if (beta[0] < 0.01) beta[0] = 0.01;
      ...
      // Get the generalized stiffness matrix for higher-order modes: Ktheta.

      getKtheta_(Ktheta, xcoords, D_mem, beta, Area);

      // Compute the higher-order membrane stiffness matrix.

      Kh_mem += ipWeights[ip] * mc3.matmul ( Ttheta_u.transpose(), Ktheta, Ttheta_u );
    }
...
void ShellANDESModel::writeLocalOutput_
...
void ShellANDESModel::getXOutTable_
...
void ShellANDESModel::getTransMatrix_
...
void ShellANDESModel::get2DLocalcoordinates_
...
{
  double x_32, x_31, x_12, x_13, x_23, x_21;
  ...
  double y_23, y_13, y_21, y_31, y_32, y_12;
  ...
  Lmat(1,1) = x_32;
  Lmat(1,2) = y_23;
  Lmat(2,0) = (1.0/6.0)*alpha_*y_23*(y_13 - y_21);
  Lmat(2,1) = (1.0/6.0)*alpha_*x_32*(x_31 - x_12);
  Lmat(2,2) = (1.0/3.0)*alpha_*(x_31*y_13 - x_12*y_21);

  Lmat(3,0) = y_31;
  Lmat(3,2) = x_13;
  Lmat(4,1) = x_13;
  Lmat(4,2) = y_31;
  Lmat(5,0) = (1.0/6.0)*alpha_*y_31*(y_21 - y_32);
  Lmat(5,1) = (1.0/6.0)*alpha_*x_13*(x_12 - x_23);
  Lmat(5,2) = (1.0/3.0)*alpha_*(x_12*y_21 - x_23*y_32);

  Lmat(6,0) = y_12;
  Lmat(6,2) = x_21;
  Lmat(7,1) = x_21;
  Lmat(7,2) = y_12;
  Lmat(8,0) = (1.0/6.0)*alpha_*y_12*(y_32 - y_13);
  Lmat(8,1) = (1.0/6.0)*alpha_*x_21*(x_23 - x_31);
  Lmat(8,2) = (1.0/3.0)*alpha_*(x_23*y_32 - x_31*y_13);
...
void ShellANDESModel::getGamma_
  ...
  
{
  ...
  double detE, JE1, JE2, JE3, JE4, JE5;

  JE1 = E11 + 2.0*E12 + E22;
  JE2 = -E12 + E33;
  JE3 = (E11 - E22)*(E11 - E22) + 4.0*(E13 + E23)*(E13 + E23);
  JE4 = (E11 - 2.0*E12 + E22 - 4.0*E33)*(E11 - 2.0*E12 + E22 - 4.0*E33) + 16.0*(E13 - E23)*(E13 - E23);
  JE5 = E11*E22*E33 + 2.0*E12*E13*E23 - E11*E23*E23 - E22*E13*E13 - E33*E12*E12;
  // JE6 = 2.0*E13*E13*E13 + E12*E13*E22 - E13*E22*E22 + E11*E11*E23 + 2.0*E13*E13*E23 + E12*E22*E23 - 2.0*E13*E23*E23 - 2.0*E23*E23*E23
  //     + 2.0*E22*(E13 + E23)*E33 - E11*(E12*E13 - E13*E22 + E12*E23 + E22*E23 + 2.0*(E13 + E23)*E33);

  detE = JE5;

  double W;

  W = (9.0*JE1*JE1*JE1 + 48.0*JE1*JE1*JE2 + JE1*(80.0*JE2*JE2 - 10.0*JE3 + JE4) + 8.0*(16.0*JE2*JE2*JE2 - JE2*(JE3 + JE4) + 72.0*JE5))/8.0;
...
void ShellANDESModel::getTthetau_
...
{
  double x_21, x_32, x_13;

  x_21 = (xcoords(0, 1) - xcoords(0, 0));
  x_32 = (xcoords(0, 2) - xcoords(0, 1));
  x_13 = (xcoords(0, 0) - xcoords(0, 2));

  double y_21, y_32, y_13;
  ...
  Ttheta_u(0,0) = x_32/(4.0*Area);
  Ttheta_u(1,0) = x_32/(4.0*Area);
  Ttheta_u(2,0) = x_32/(4.0*Area);
  Ttheta_u(0,1) = y_32/(4.0*Area);
  Ttheta_u(1,1) = y_32/(4.0*Area);
  Ttheta_u(2,1) = y_32/(4.0*Area);
  Ttheta_u(0,2) = 1.0;

  Ttheta_u(0,3) = x_13/(4.0*Area);
  Ttheta_u(1,3) = x_13/(4.0*Area);
  Ttheta_u(2,3) = x_13/(4.0*Area);
  Ttheta_u(0,4) = y_13/(4.0*Area);
  Ttheta_u(1,4) = y_13/(4.0*Area);
  Ttheta_u(2,4) = y_13/(4.0*Area);
  Ttheta_u(1,5) = 1.0;

  Ttheta_u(0,6) = x_21/(4.0*Area);
  Ttheta_u(1,6) = x_21/(4.0*Area);
  Ttheta_u(2,6) = x_21/(4.0*Area);
  Ttheta_u(0,7) = y_21/(4.0*Area);
  Ttheta_u(1,7) = y_21/(4.0*Area);
  Ttheta_u(2,7) = y_21/(4.0*Area);
  Ttheta_u(2,8) = 1.0;
}
...
void ShellANDESModel::getKtheta_
  ...
  
{
  ...
  Matrix D_mem_nat   ( strCount_, strCount_ );

  getDmemNat_(D_mem_nat, xcoords, D_mem, Area);
  ...
  double l_21, l_32, l_13;

  l_21 = sqrt(pow(x_21,2) + pow(y_21,2));
  l_32 = sqrt(pow(x_32,2) + pow(y_32,2));
  l_13 = sqrt(pow(x_13,2) + pow(y_13,2));

  Matrix Q1   ( strCount_, nodeCount_ );
  Matrix Q2   ( strCount_, nodeCount_ );
  Matrix Q3   ( strCount_, nodeCount_ );

  Q1(0,0) = beta[1]/pow(l_21,2);
  Q1(0,1) = beta[2]/pow(l_21,2);
  Q1(0,2) = beta[3]/pow(l_21,2);
  Q1(1,0) = beta[4]/pow(l_32,2);
  Q1(1,1) = beta[5]/pow(l_32,2);
  Q1(1,2) = beta[6]/pow(l_32,2);
  Q1(2,0) = beta[7]/pow(l_13,2);
  Q1(2,1) = beta[8]/pow(l_13,2);
  Q1(2,2) = beta[9]/pow(l_13,2);
  Q1 *= (2.0*Area/3.0);

  Q2(0,0) = beta[9]/pow(l_21,2);
  Q2(0,1) = beta[7]/pow(l_21,2);
  Q2(0,2) = beta[8]/pow(l_21,2);
  Q2(1,0) = beta[3]/pow(l_32,2);
  Q2(1,1) = beta[1]/pow(l_32,2);
  Q2(1,2) = beta[2]/pow(l_32,2);
  Q2(2,0) = beta[6]/pow(l_13,2);
  Q2(2,1) = beta[4]/pow(l_13,2);
  Q2(2,2) = beta[5]/pow(l_13,2);
  Q2 *= (2.0*Area/3.0);

  Q3(0,0) = beta[5]/pow(l_21,2);
  Q3(0,1) = beta[6]/pow(l_21,2);
  Q3(0,2) = beta[4]/pow(l_21,2);
  Q3(1,0) = beta[8]/pow(l_32,2);
  Q3(1,1) = beta[9]/pow(l_32,2);
  Q3(1,2) = beta[7]/pow(l_32,2);
  Q3(2,0) = beta[3]/pow(l_13,2);
  Q3(2,1) = beta[1]/pow(l_13,2);
  Q3(2,2) = beta[2]/pow(l_13,2);
  Q3 *= (2.0*Area/3.0);

  Matrix Q4   ( strCount_, nodeCount_ );
  Matrix Q5   ( strCount_, nodeCount_ );
  Matrix Q6   ( strCount_, nodeCount_ );
  ...
  Ktheta = 0.0;
  Ktheta += mc3.matmul ( Q4.transpose(), D_mem_nat, Q4 );
  Ktheta += mc3.matmul ( Q5.transpose(), D_mem_nat, Q5 );
  Ktheta += mc3.matmul ( Q6.transpose(), D_mem_nat, Q6 );
}
...
void ShellANDESModel::getDmemNat_
  ...
  
{
  ...
  double x_23, x_13, x_31, x_21, x_12, x_32;
  ...
  double y_23, y_13, y_31, y_21, y_12, y_32;
  ...
  Matrix Te   ( strCount_, strCount_ );

  Te(0,0) = y_23*y_13*pow(l_21,2);
  Te(0,1) = y_31*y_21*pow(l_32,2);
  Te(0,2) = y_12*y_32*pow(l_13,2);

  Te(1,0) = x_23*x_13*pow(l_21,2);
  Te(1,1) = x_31*x_21*pow(l_32,2);
  Te(1,2) = x_12*x_32*pow(l_13,2);

  Te(2,0) = (y_23*x_31 + x_32*y_13)*pow(l_21,2);
  Te(2,1) = (y_31*x_12 + x_13*y_21)*pow(l_32,2);
  Te(2,2) = (y_12*x_23 + x_21*y_32)*pow(l_13,2);

  Te *= 1.0/(4.0*pow(Area,2));

  D_mem_nat = mc3.matmul ( Te.transpose(), D_mem, Te );
}
...
void ShellANDESModel::getMembraneDisp_
...
void ShellANDESModel::getHmatAnalytic_
...
void ShellANDESModel::getHmatNumeric_
...
void ShellANDESModel::getDissForce_
...
void ShellANDESModel::initCharLength_ ()
...
void ShellANDESModel::getDissipation_
...
void ShellANDESModel::getIshape_ 
...
void ShellANDESModel::getMaterial_
...
void ShellANDESModel::getThickness_
...
void ShellANDESModel::getOrientVec_
...
void ShellANDESModel::getStress_
...
void ShellANDESModel::writeElements_
...
void ShellANDESModel::writeNodes_
...
void  ShellANDESModel::initLocOutWriter_ ()
...
Ref<PrintWriter>  ShellANDESModel::initWriter_
...
void ShellANDESModel::writeDisplacements_
...
bool ShellANDESModel::getTable_
...
static Ref<Model>     newShellANDESModel
...
{
  return newInstance<ShellANDESModel> ( name, conf, props, globdat );
}
...
void declareShellANDESModel ()
{
  using jive::model::ModelFactory;

  ModelFactory::declare ( "ShellANDES", & newShellANDESModel );
}