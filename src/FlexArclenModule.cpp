...

{
  ...
  if ( converged )
  {
    ...
    params.set( SolverNames::CHECK_BOUNDS, ! keepLargeTmp_ );
    ...
    params.find ( newCh,   SolverNames::CHANGE_COUNT );
  ...
  else
  {
    ...
    else if ( doneTmpNonlin_ == 1 )
    {
      ...
      backup.set ( SolverNames::STEP_SIZE, arcBackup_ );

      model_->takeAction ( SolverNames::SET_STEP_SIZE, backup, globdat );
    ...
    else if ( turnOffResSwt_ ( globdat ) )
    {
      System::out() << "Returning tot the same load step,\n " 
                    << "now without residual based switch in XArclenModule\n";
    }
...

{
  ...
  model_->takeAction ( SolverNames::GET_STEP_SIZE, params, globdat );

  if ( params.find ( inc, SolverNames::STEP_SIZE ) )
  {
...

{
  ...
  if ( noResSwitch_ ) 
  {
    // noResSwitch option was succesfull, restore the switch

    noResSwitch_ = false;

    arclenSolver_->setSwtResidual ( swtResidual0_ );
  }
  ...
  if ( isCareful_ )
  {
    Properties params;
    isCareful_ = false;
    model_->takeAction ( SolverNames::STOP_CAREFUL, params, globdat );
  }
...

{
  ...
  params.find ( modelAllowsIt, SolverNames::ALLOW_TMP );
         ...
         modelAllowsIt                    )
  {
    ...
    Properties backup;
    model_->takeAction ( SolverNames::GET_STEP_SIZE, backup, globdat );
    backup.get ( arcBackup_, SolverNames::STEP_SIZE );

    params.set ( SolverNames::TEMPORARY, true );
...

{
  ...
  if ( arclenTriedSma_ )
  {
    params.set ( SolverNames::ADAPT_HOW,  "increase"   );
    params.set ( SolverNames::ADAPT_FROM, maxArcTried_ );

    model_->takeAction ( SolverNames::ADAPT_STEP, params, globdat );
  }
...

{
  ...
  params.set ( SolverNames::ADAPT_HOW, "reduce" );
...

{
  ...
  if ( ! arclenTriedAll_ )
  {
    ...
    params.find ( increased, SolverNames::DONE );
...

{
  ...
  // store input treshold value 

  swtResidual0_ = arclenSolver_->getSwtResidual();

  // set treshold to very high value

  arclenSolver_->setSwtResidual ( maxOf ( swtResidual0_ ) );
...

{
  ...
  isCareful_ = model_->takeAction 
    ( SolverNames::BE_CAREFUL, actionParams, globdat );
...

{
  ...
  globdat.find ( fDiss, SolverNames::DISSIPATION_FORCE );
  ...
  if ( fDiss.size() != dofs->dofCount() )
  {
    ...
    globdat.set  ( SolverNames::DISSIPATION_FORCE, fDiss );
  }

  fDiss = 0.;

  model_->takeAction ( SolverNames::GET_DISS_FORCE, params, globdat );
}
...

{
  ...
  Ref<XArclenModule> solver2 = newInstance<XArclenModule> 
