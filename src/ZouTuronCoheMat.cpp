 ...
 *  with friction in the damaged part (Zou 2018)
...
const char*  ZouTuronCoheMat::F2T_PROP      = "f2t";
const char*  ZouTuronCoheMat::F6_PROP       = "f6";
const char*  ZouTuronCoheMat::G_I_PROP      = "gI";
const char*  ZouTuronCoheMat::G_II_PROP     = "gII";
const char*  ZouTuronCoheMat::ETA_PROP      = "eta";
const char*  ZouTuronCoheMat::DUMMY_PROP    = "dummy";
const char*  ZouTuronCoheMat::MU_PROP       = "mu";
const char*  ZouTuronCoheMat::DILATANT_PROP = "dilatancy";

const double ZouTuronCoheMat::OMEGA_MAX     = 1. - 1.e-12;
const double ZouTuronCoheMat::EPS           = 1.e-10;

//-----------------------------------------------------------------------
//   constructors & destructor
//-----------------------------------------------------------------------

ZouTuronCoheMat::ZouTuronCoheMat
...
void ZouTuronCoheMat::configure
...
void ZouTuronCoheMat::getConfig
      ...
      idx_t                 mpoint )
{
  ...
  updateZou_ ( traction, stiff, dDdu, damage, jump, mpoint );
...
double  ZouTuronCoheMat::updateTuron_
...

{
  ...
  // NB: limited interpenetration penalty here, that is completed in the friction part

  stiff = 0.;

  stiff(0,0) = ( 1. - hisDam ) * dummy_;
  ...
  if ( loading )
  {
    ...
    double dDddeq = 1. / ( deltaF - delta0 );
    double ddddeq = dDddeq * deltaF * delta0 / delta / delta;

    dDdu = ( dDddeq / delta ) * jumpCorr;
    dddu = ( ddddeq / delta ) * jumpCorr;

    double tmp    = dDddeq * dDddeq;
    double dDdd0  = ( delta - deltaF ) * tmp;
    double dDddF  = ( delta0 - delta ) * tmp;
    double dddd0  = dDdd0 * deltaF / delta;
    double ddddF  = dDddF * delta0 / delta;
    ...
    if ( deltaS > EPS )
    {
      ...
      if (   rank_ == 2 )
      {
         dBdu[1]  = - dBdu[0] * jumpCorr[0] / jumpCorr[1];
      }
      else
      {
         dBdu[1]  = 2 * tmp * tmp * B * B * jump[1];
         dBdu[2]  = 2 * tmp * tmp * B * B * jump[2];
      }
    }
    else
    {
      dBdu[0] = 0.;
      dBdu[slice(1,END)] = 1. / jumpCorr[0] / jumpCorr[0];
    }
    ...
    stiff(i,j)   += dtdd[i] * dddu[j] + dtdD[i] * dDdu[j];
  }
  else
  {
    dDdu = 0.;
...
bool  ZouTuronCoheMat::updateZou_
...
void  ZouTuronCoheMat::elasticUpdate
...
void  ZouTuronCoheMat::getDissForce
      ...
      const idx_t   mpoint )
{
  ...
  bool friction = updateZou_ ( ftraction, fstiff, dDdu, damage, jump, mpoint );
...
void  ZouTuronCoheMat::allocPoints 
...
void  ZouTuronCoheMat::allocPoints_
...
void ZouTuronCoheMat::deallocPoints ( idx_t count )
{
...
double  ZouTuronCoheMat::getDelta0_ ( const double beta ) const
