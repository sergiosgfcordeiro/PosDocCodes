...

{
  ...
  if ( action == SolverNames::GET_STEP_SIZE )
  {
...

{
  ...
  if ( ! isDispControl_ )
  {
    ...
    if ( dispBC_->getLoadScale() * dispBC_->getLoadScale0() < 0. )
    {
      ...
      params.set ( SolverNames::DISCARD, true );
    }
    ...
    // set flag, tmpNonlin in FlexArclenModule should only be allowed
    // when the energy increment is small

    bool allowTmpNonlin = ( arcLength_ < swtEner_ );

    params.set ( SolverNames::ALLOW_TMP, allowTmpNonlin );
  }
}
...

{
  ...
  if ( isDispControl_ && ! tempStep_ )
  {
    ...
    if ( isTmpDisp_ )
    {
      params.find ( doCheck, SolverNames::CHECK_BOUNDS );

      if ( doCheck && lastArcl_ > maxIncr_ )
      {
        System::warn() << "Energy increment too large. Solution discarded.\n"
          << "This check is optional. " << SolverNames::CHECK_BOUNDS << '\n';
  ...
  else
  {
    ...
    if ( preferDisp_ )
    {
      ...
      if ( ( dlambda > 0. ) && ( arcLength_ <= swtEner_ ) )
      {
        ...
        params.set ( SolverNames::DO_SWITCH, "ifAccept" );
...

{
  ...
  params.set ( SolverNames::STEP_SIZE, incr );
}
...
{
  double incr;
  params.get ( incr, SolverNames::STEP_SIZE );
...

{
  ...
  params.get ( maxTried, SolverNames::ADAPT_FROM );
...

{
  ...
  params.find ( temporary, SolverNames::TEMPORARY );
...
//   idx_t nLoaded = dispBC_->getLoadedDofs ( idofs );
  ...
  // System::out() << "dEnergy: u " << uB << " u0 " << u0B << " du " << du
  ...
  if ( globdat.find ( fDiss, SolverNames::DISSIPATION_FORCE ) )
  {
    ...
    double duFDiss = dot ( du, fDiss );

    dEnergy += 0.5 * duFDiss;

    // System::out() << .5*duFDiss << " of " << dEnergy << " is plastic"
      // << ", i.e. " << .5*duFDiss/dEnergy*100. << '%' << endl;
    ...
    // NB: elastic energy itself (energy0_) is not adapted,
    // because this would require another vector assembly
    // Only the incremental stuff is really necessary
  }
