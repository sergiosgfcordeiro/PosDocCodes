...
Ly = 12.0;  // Length in y-direction

// Mesh resolution (number of divisions)
Nx = 129;  // Divisions along the x-axis
Ny = 33;   // Divisions along the y-axis

// Define points for the rectangle
Point(1) = {0, 0, 0, 1};   // Bottom-left
Point(2) = {Lx, 0, 0, 1};  // Bottom-right 
Point(3) = {0, Ly, 0, 1}; // Top-left 
Point(4) = {Lx, Ly, 0, 1};  // Top-right 

// Define the lines for the rectangle
Line(1) = {1, 2};  // Bottom side
Line(2) = {2, 4};  // Right side
Line(3) = {4, 3};  // Top side
Line(4) = {3, 1};  // Left side
...
Physical Surface("beam") = {1};  // Defines the panel
